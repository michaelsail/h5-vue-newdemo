  ...
  <div>
    <div class="hallDocItem" @click="goNativeDoc">
      ...
      <div class="relative flex1 pt20 pb20 bt_gray">
        ...
        <div class="fs14 c999 typo_bold mb15 elli w220">
          {{docItem.unit_name}}
        </div>
        ...
        <template v-if="isFilterIndex">
          <div class="collect filterIndex" @click.stop="select(docItem.account_user_id)">
            选择
          </div>
        </template>
      </div>
...
<script>
import { pullDocIndex, chooseDoctor } from '@/mixins/pullNativeFunc'
import service from '_services/'
export default {
  mixins: [pullDocIndex, chooseDoctor],
  props: {
    isHall: {
      type: [Boolean],
      default: true
  ...
  methods: {
    select(account_user_id){
      console.log('选择');
      this.chooseDoctor(account_user_id)
    },
    goNativeDoc() {
